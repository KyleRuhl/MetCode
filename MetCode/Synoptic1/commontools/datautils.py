...
E5_varnames = ['T', 'Z', 'U', 'V', 'PV', 'Mont', 'Vort', 'Pres', 'SH', 'PS', 'PSL', 'T2m']
...
url_templates = {'GFS'      :'https://www.ncei.noaa.gov/thredds/fileServer/model-gfs-g4-anl-files-old/{y}{m:02d}/{y}{m:02d}{d:02d}/gfsanl_4_{y}{m:02d}{d:02d}_{s:02d}00_00{f:d}.grb2',
                 'GFS_wget1':'https://www.ncei.noaa.gov/data/global-forecast-system/access/grid-004-0.5-degree/analysis/{y}{m:02d}/{y}{m:02d}{d:02d}/gfs_4_{y}{m:02d}{d:02d}_{s:02d}00_00{f:d}.grb2',
                 'GFS_wget2':'https://www.ncei.noaa.gov/data/global-forecast-system/access/historical/analysis/{y}{m:02d}/{y}{m:02d}{d:02d}/gfsanl_4_{y}{m:02d}{d:02d}_{s:02d}00_00{f:d}.grb2',
                 'NAM'      :'https://www.ncei.noaa.gov/thredds/fileServer/model-nam218-old/{y}{m:02d}/{y}{m:02d}{d:02d}/nam_218_{y}{m:02d}{d:02d}_{s:02d}00_00{f:d}.grb2',
                 'NAM_wget1':'https://www.ncei.noaa.gov/data/north-american-mesoscale-model/access/analysis/{y}{m:02d}/{y}{m:02d}{d:02d}/namanl_218_{y}{m:02d}{d:02d}_{s:02d}00_00{f:d}.grb2',
                 'NAM_wget2':'https://www.ncei.noaa.gov/data/north-american-mesoscale-model/access/historical/analysis/{y}{m:02d}/{y}{m:02d}{d:02d}/namanl_218_{y}{m:02d}{d:02d}_{s:02d}00_00{f:d}.grb2',
...
def guess_cache_locations():
   read_cache_checks = ['/scratch/eas3421/adm_data/main/',
                        '/scratch/eas3421/2022S/{user}/adm_data/',
                        '/data/{user}/adm_cache/']

   write_cache_checks = ['/scratch/eas3421/2022S/{user}/adm_data/',
                         '/data/{user}/adm_cache/']
...
def validate_ds(dataset):
   dss = list(fname_templates.keys()) + list(url_templates.keys())
...
def get_analysis(dataset, start_date, end_date, varlist, time_step = 3, refresh = False):
   ...
   if 'E5' in dataset:
      valid_vars = E5_varnames
   else:
      valid_vars = GFS_thredds_varnames.keys()
      can_fetch = True

   # Validate requested variables
   for v in varlist:
      if not v in valid_vars:
         raise ValueError('Unrecognized variable %s. Options are %s.' % (v, valid_vars))
   ...
   for d in dates:
      ...
      if len(tofetch) > 0:
         if not can_fetch:
            raise ValueError('Some data from the requested date range is unavailable for the %s dataset.' % dataset)

         try:
            print("Trying to grab data from THREDDS")
            fetch_analysis(dataset, year, month, day, hour, tofetch)
            print('THREDDS Successful!')
         except:
            print("THREDDS failed, trying direct download")
...
def fetch_analysis_wget(dataset, year, month, day, hour, varlist):
   ...
   dsets = [k for k in url_templates.keys() if dataset in k]
   print(dsets)

   success = False
   for ds in dsets:
      url = generate_url(ds, year, month, day, syn, fc)
      ...
      try:
         ...
         print('Downloading {ds} Analysis for {y}-{m:02d}-{d:02d} {h:02d} failed ({msg}).'.format(ds = ds, y = year, m = month, d = day, h = hour, msg = e))

   if not success:
      raise OSError('Failed to download {ds} from any source. Aborting.'.format(ds = dataset))
